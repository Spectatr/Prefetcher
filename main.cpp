		...
		for (int index2 = 0; index2 <= 4; ++index2)
		{
			...
			//		printf("%u: %u\n",curr_cycle,cpu_status);
				...
				*/
				...
				char* outfile = (char *)malloc(sizeof(char)*(strlen(argv)+5));
				strcpy(outfile,argv);
				strcat(outfile,".out");
				...
				fprintf(fp,"%-35s %10.4f\n", "Average Memory Access Time",cpu.getAMAT());
			...
			if (bestavg > sumavg)
			{
				bestavg = sumavg;
				printf("\n\n\nTotal average: %lf\n", sumavg);
				printf("GlobalHistory: %d, %d\n", GlobalHistory::index1, GlobalHistory::index2);
			}
		}
	}
	return 0;
